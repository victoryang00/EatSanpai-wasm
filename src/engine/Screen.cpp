...
void Screen::DrawCentered(const std::string &filename, const double x, const double y) {
    ...
    jngl::draw(filepath, x - jngl::getWidth(filepath) / 2.0, y - jngl::getHeight(filepath) / 2.0);
}
...
Screen &GetScreen() { return *Screen::handle(); }

void Screen::printCentered(const std::string &text, const jngl::Vec2 center) {
    jngl::print(text, static_cast<int>((center.x - jngl::getTextWidth(text) / 2)),
